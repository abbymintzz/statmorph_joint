""" 
Slightly modified version of statmorph by Abby Mintz
Modified to allow for joint fitting of two images, e.g. Halpha and z-band 
such that the centers (centroid, asymmetry center, m20 center) and segmaps (gini seg map)
are fixed to one image. 

The function `source_morphology_joint` is the main function to use.
The function `source_morphology` is the original function, which will perform 
identically to statmorph (v 0.6.1). The only difference is that it will have three 
extra properties (_m20_center, xc_m20, yc_m20). 
"""
...
class SourceMorphology(object):
    ...
    def __init__(self, image, segmap, label, mask=None, weightmap=None,
        ...
        # Fix properties to provided values, if any:
...
# HERE
    @lazyproperty
    def _m20_center(self):
        """
        The (xc, yc) center of the region used for M_20 calculation.
        """
    ...
    def xc_m20(self):
        """
        The x-coordinate of the m20 center, relative to the original image.
        """
        return self._m20_center[0] + self.xmin_stamp
    ...
    def yc_m20(self):
        ...
        The x-coordinate of the m20 center, relative to the original image.
        """
        return self._m20_center[1] + self.xmin_stamp
    ...
    def m20(self):
        ...
        image = np.float64(image)  # skimage wants double
# HERE 
        # THIS IS COMMENTED OUT SO THE CENTER CAN BE FIXED
        # Calculate centroid
        # M = skimage.measure.moments(image, order=1)
        # if M[0, 0] <= 0:
        #     warnings.warn('[deviation] Nonpositive flux within Gini segmap.',
...
def source_morphology_joint(images, segmap, weightmaps, **kwargs):
    ...
    sources_morph = [fix_morph]
    for image, wm in zip(images[1:], weightmaps[1:]):
        sources_morph.append(SourceMorphology(image, segmap, segmap.labels[0], weightmap = wm, fixed_vals=fixed_vals, **kwargs))
